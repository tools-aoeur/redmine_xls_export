class XlsExportController < ApplicationController
  ...
  def index
    ...
    if request.post?
      ...
      if retrieve_xls_export_data(@settings)
        ...
        send_data(export_to_string(export_name), type: export_name[1].to_sym,
                                                 filename: filename_for_content_disposition(export_name.join('.')))
      else
  ...
  def query_issues(export_offset, limit)
    ...
    if (Redmine::VERSION::MAJOR <= 3) && (Redmine::VERSION::MINOR <= 3) && (Redmine::VERSION::BRANCH != 'devel')
      options.merge!({ include: %i[assigned_to tracker priority category fixed_version] })
    end
  ...
  def export_to_string(export_name)
    ...
    zip_file = Zip::ZipOutputStream.write_buffer do |zip_stream|
      ...
      @issues.each do |issue|
        if @settings['export_attached'] == '1'
          retrieve_attachments_list(issue).each do |attach|
            ...
            zip_stream.put_next_entry(
              format(
                "#{file ? "#{ATTACHMENTS_FOLDER}/" : "#{NOT_FOUND_ATTACHMENTS_FOLDER}/"}%05i/#{create_zip_filename(attach)}", issue.id
              ), nil, nil, Zip::ZipEntry::DEFLATED, Zlib::BEST_COMPRESSION
            )
            unless file
              file = StringIO.new('')
              file.write("\n")
        ...
        next unless @settings['separate_journals'] == '1'

        journal_xls = journal_details_to_xls(issue, @settings)
        next unless journal_xls

        zip_stream.put_next_entry(format("#{JOURNALS_FOLDER}/%05i_journal_details.xls", issue.id), nil, nil,
                                  Zip::ZipEntry::DEFLATED, Zlib::BEST_COMPRESSION)
  ...
  def create_zip_filename(attach)
    new_name = ''
    new_name << if attach[:idx] == 0
                  attach[:object].filename
                elsif attach[:object].filename =~ /^(.+)\.([^ .]+?)$/
                  format("#{::Regexp.last_match(1)}(%02i).#{::Regexp.last_match(2)}", attach[:idx].to_i)
                else
                  format("#{attach[:object].filename}(%02i)", attach[:idx].to_i)
                end

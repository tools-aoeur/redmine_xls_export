class XlsExportHook < Redmine::Hook::ViewListener
  def view_issues_index_bottom(context = {})
    return unless context[:query].valid? && !context[:issues].empty?
    ...
    ret_str << content_tag('span', link_to(l(:label_plugin_xlse_export_format_quick),
                                           hook_url_for({ controller: 'xls_export', action: 'export_current',
                                                          project_id: context[:project] }),
                                           { class: 'xls', rel: 'nofollow',
                                             title: l(:label_plugin_xlse_export_format_quick_tooltip) }))
    ret_str << content_tag('span', link_to(l(:label_plugin_xlse_export_format_detailed),
                                           hook_url_for({ controller: 'xls_export', action: 'index',
                                                          project_id: context[:project] }),
                                           { class: 'xlse',
                                             title: l(:label_plugin_xlse_export_format_detailed_tooltip) }))
    ret_str << '</p>'
  ...
  def suburi(url)
    ...
    url = baseurl + url unless url.match(/^#{baseurl}/)
    url
  end
